...
function RssNewsDisplay($setting) 
{
	...
	if ( ! empty($itemblocks) ) 
	{
		...
		{ 
			...
			preg_match_all( "/\<pubDate\>(.*?)\<\/pubDate\>/", $block, $date );	
			preg_match_all( '/<category domain="http:\/\/www.ilsole24ore.com"\>(.*?)<\/category>/', $block, $category );
			$rssnews_title = $title[1][0];
			$rssnews_title = mysql_real_escape_string(trim($rssnews_title));
			$rssnews_title = str_replace("\\", "",$rssnews_title);
			$rssnews_link = $link[1][0];
			$rssnews_category = $category[1][0];
			$rssnews_link = trim($rssnews_link);						
			$rssnews_date = $date[1][0];						
			$rssnews_date = trim($rssnews_date);						
			$rssnews_date=substr($rssnews_date,5);			
			$rssnews_date=substr($rssnews_date,0,-13);			
			$en = array("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"); 			
			$it = array("01","02","03","04","05","06","07","08","09","10","11","12"); 			
			$rssnews_date = str_replace($en, $it, $rssnews_date);  			
			$rssnews_date = str_replace(" ", ".", $rssnews_date);
			$oggi =(date("d.m.Y"));
			list($gg, $mm, $yy) = explode('.', $oggi);
			$timestamp_oggi = mktime(0, 0, 0, $mm, $gg, $yy);
			list($gg, $mm, $yy) = explode('.', $rssnews_date);
			$timestamp_rssnews_date = mktime(0,0,0, $mm, $gg, $yy);
			$diff = $timestamp_oggi-$timestamp_rssnews_date;
			$un_mese = 2592000;
			$tre_mesi = $un_mese*3;
			if ($diff < $tre_mesi) 
				{
